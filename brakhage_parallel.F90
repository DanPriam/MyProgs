program main
	...
	if (rank_cart == 0) then
		allocate(x_model(N))
		do i = 1,N
			x_model(i) = real(sin(2*pi*(i-1)/(N-1)), kind(0q0))
		enddo
	end if
	
	if (rank_cart < num_col) then
		call MPI_Scatterv(x_model,rcounts_N,displs_N,MPI_REAL16,&
	...
	call Brakhage_parallel(A_part,b_part,x_part,delta2, &
					 M_part, N_part, &
					 comm_row, comm_col,  comm_cart, N, nu)
...
subroutine Brakhage_parallel(A_part,b_part,x_part,delta2, &
					 M_part, N_part, &
					 comm_row, comm_col, comm_cart, N, nu)
	...
	integer  :: M_part, N_part, N, s, i, j
	real(8) :: A_part(M_part,N_part),x_part(N_part),b_part(M_part)
	real(8) :: normA_part, normAsq, normA
	real(8) :: delta2
	real(8) :: w
	real(8) :: mu
	real(8) :: nu
	...
	call MPI_Allreduce_init(Ax_part_temp, Ax_part, &
							M_part, MPI_REAL8, &
							MPI_SUM, comm_row, info, requests(0), ierr)
	call MPI_Allreduce_init(criterion_temp, criterion, &
							1, MPI_REAL8, &
							MPI_SUM, comm_col, info, requests(1), ierr)
	call MPI_Allreduce_init(gr_part_temp, gr_part, &
							N_part, MPI_REAL8, &
							MPI_SUM, comm_col, info, requests(2), ierr)
	...
	call MPI_Allreduce(normA_part, normA, &
							1, MPI_REAL8, &
							MPI_SUM, comm_cart, ierr)
	normA = sqrt(normAsq)
	A_part = A_part*(0.99/normA)
	b_part = b_part*(0.99/normA)
	delta2 = delta2 * ((0.99/normA)**2)
	...
	do while (.TRUE.)
		...
		if (s == 1) then
			...
			call MPI_Start(requests(0), ierr)
			call MPI_Wait(requests(0), status, ierr)
			b_part = b_part - Ax_part
			gr_part_temp = matmul(transpose(A_part), b_part)
